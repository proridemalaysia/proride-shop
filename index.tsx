...
const App = () => {
  ...
  // Check if any shirt size has stock
  const isAnyShirtAvailable = useMemo(() => {
      return SHIRT_SIZES.some(size => (stockLevels[`GIFT-SHIRT-${size}`] ?? 0) > 0);
  }, [stockLevels]);
  ...
  const initiatePayment = async () => {
    ...
    // Updated validation: Require shirt size only if stock exists
    if (hasShirtGift && isAnyShirtAvailable && !selectedShirtSize) return setErrorMsg("Select T-Shirt Size");
    setIsProcessingPayment(true);
    
    const gifts = ['Proride Sticker'];
    if (hasShirtGift) {
        if (selectedShirtSize) {
            gifts.push(`Proride T-Shirt (${selectedShirtSize})`);
        } else if (!isAnyShirtAvailable) {
            gifts.push(`Proride T-Shirt (Out of Stock)`);
        }
    }
                        ...
                        <div className="space-y-2 mt-2">
                            ...
                            {isAnyShirtAvailable ? (
                                ...
                                >
                                 ...
                                 <div className="text-xs text-orange-600 bg-orange-50 border border-orange-100 p-2 rounded flex items-center justify-center italic">
                                     Free T-Shirts are currently out of stock. You can still proceed.
                                 </div>
                             )}
                        </div>
                    )}
